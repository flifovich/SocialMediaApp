...
export function Post({ post, setPosts}: PostProps) {
    ...
    const [postLiked, setPostLiked] = useState<boolean>(
        Boolean(post.likes?.some((like) => like.id === user?.id))
    );



    useEffect(() => {
        setPostLiked(Boolean(post.likes?.some((like) => like.id === user?.id)))
    }, [post.likes, user?.id]);

    const like = async () => {
        // update ui, if theres error revert back
        setPostLiked((prev) => !prev);
        try{
            const response = await fetch(
                import.meta.env.VITE_API_URL + "/feed/posts/" + post.id + "/like",{
                    method: "PUT",
                    headers: {
    ...
    const editComment = async (id: number, content: string) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/feed/comments/${id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
    ...
    const editPost = async (content: string, picture: string) => {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/feed/posts/${post.id}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
        ...
        <div className={classes.root}>
            ...
            <div className={classes.stats}>
                {post.likes && post.likes.length > 0 ? (
                    ...
                    <div className={classes.stat}>Be first to like</div>
                )}

                {post.comments && post.comments.length > 0 ? (
                    <button className={classes.stat} onClick={() => setShowComments((previous) => !previous)}>
